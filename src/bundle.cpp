...
bool ZBundle::VerifySignedBundle(bool bCheckCMS)
{
	if (m_strAppFolder.empty()) {
	...
	for (const string& strFile : arrMachOFiles) {
		ZMachO macho;
		if (!macho.Init(strFile.c_str())) {
			continue;
		}
		if (!macho.VerifyCodeSlots(true)) {
	...
	string strInfoPath = m_strAppFolder + "/Info.plist";
	string strInfoPlist;
	...
	string strMainBinary = m_strAppFolder + "/" + strExecName;
	ZMachO mainMacho;
	if (!mainMacho.Init(strMainBinary.c_str())) {
		ZLog::ErrorV(">>> Verify: can't open main binary %s\n", strExecName.c_str());
		return false;
	}

	if (!mainMacho.VerifyEmbeddedSignature(bCheckCMS)) {
